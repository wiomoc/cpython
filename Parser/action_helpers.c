...
NameDefaultPair *
_PyPegen_name_default_pair(Parser *p, arg_ty arg, expr_ty value, int is_defered, Token *tc)
{
...
static asdl_arg_default_seq *
_get_defaults(Parser *p, asdl_seq *names_with_defaults)
{
    Py_ssize_t len = asdl_seq_LEN(names_with_defaults);
    asdl_arg_default_seq *seq = _Py_asdl_arg_default_seq_new(len, p->arena);
    if (!seq) {
        return NULL;
    }
    for (Py_ssize_t i = 0; i < len; i++) {
        NameDefaultPair *pair = asdl_seq_GET_UNTYPED(names_with_defaults, i);
        if (pair->value) {
            arg_default_ty arg_default = _PyAST_arg_default(pair->value,
                pair->is_defered, p->arena);
            asdl_seq_SET(seq, i, arg_default);
        } else {
            asdl_seq_SET(seq, i, NULL);
        }
    }
    return seq;
}
...
_make_posdefaults(Parser *p,
                  SlashWithDefault *slash_with_default,
                  asdl_seq *names_with_default,
                  asdl_arg_default_seq **posdefaults) {
    if (slash_with_default != NULL && names_with_default != NULL) {
        asdl_arg_default_seq *slash_with_default_values =
                _get_defaults(p, slash_with_default->names_with_defaults);
        if (!slash_with_default_values) {
            return -1;
        }
        asdl_arg_default_seq *names_with_default_values = _get_defaults(p, names_with_default);
        if (!names_with_default_values) {
            return -1;
        }
        *posdefaults = (asdl_arg_default_seq*)_PyPegen_join_sequences(
                p,
        ...
        *posdefaults = _Py_asdl_arg_default_seq_new(0, p->arena);
    }
...
_make_kwargs(Parser *p, StarEtc *star_etc,
             ...
             asdl_arg_default_seq **kwdefaults) {
    ...
    if (star_etc != NULL && star_etc->kwonlyargs != NULL) {
        ...
        *kwdefaults = _Py_asdl_arg_default_seq_new(0, p->arena);
    }
...
_PyPegen_empty_arguments(Parser *p)
{
    ...
    asdl_arg_default_seq *posdefaults = _Py_asdl_arg_default_seq_new(0, p->arena);
    if (!posdefaults) {
        return NULL;
    }
    ...
    asdl_arg_default_seq *kwdefaults = _Py_asdl_arg_default_seq_new(0, p->arena);
    if (!kwdefaults) {
        return NULL;
    }
